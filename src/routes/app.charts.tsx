...
import { Search, Activity, Layers, BarChart3, ChevronUp, ChevronDown } from "lucide-react";

export const Route = createFileRoute("/app/charts")({ component: ChartView });

const TIMEFRAMES = ["1m", "5m", "15m", "1h", "4h", "D", "W"];

function genCandles(seed: string, count = 180, basePrice = 100) {
  let h = 0; for (let i = 0; i < seed.length; i++) h = (h * 31 + seed.charCodeAt(i)) >>> 0;
  const data: any[] = []; let p = basePrice; const start = Math.floor(Date.now() / 1000) - count * 3600;
  for (let i = 0; i < count; i++) {
    h = (h * 1103515245 + 12345) >>> 0;
    const drift = (((h % 1000) / 1000) - 0.5) * basePrice * 0.015;
    ...
    data.push({ time: start + i * 3600, open, high, low, close, value: volume, color: close >= open ? "rgba(0, 212, 170, 0.5)" : "rgba(239, 68, 68, 0.5)" });
    p = close;
  }
  return data;
}
...
function ChartView() {
  ...
  useEffect(() => {
    ...
    const data = genCandles(ticker + tf, 180, q.prevClose);
        ...
        <div className="flex items-center gap-0.5 p-0.5 rounded-md border border-border bg-background">
          {TIMEFRAMES.map(t => (
            <button key={t} onClick={() => setTf(t)}
              className={`h-6 px-2 rounded text-xs font-mono ${tf === t ? "bg-brand text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
              {t}
            </button>
          ))}
        </div>
        <button className="h-8 px-2.5 rounded-md border border-border bg-background hover:bg-accent text-xs flex items-center gap-1.5"><BarChart3 className="size-3.5" /> Candles</button>
        <button className="h-8 px-2.5 rounded-md border border-border bg-background hover:bg-accent text-xs flex items-center gap-1.5"><Activity className="size-3.5" /> VWAP · EMA · BB</button>
      </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {showL2 && q && (
            <div className="h-56 border-t border-border bg-surface grid grid-cols-2 shrink-0">
              <L2Book price={q.price} side="bid" />
              <TimeSales ticker={ticker} basePrice={q.price} />
            </div>
          )}
          <button onClick={() => setShowL2(!showL2)} className="h-7 border-t border-border bg-surface hover:bg-accent flex items-center justify-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
            {showL2 ? <ChevronDown className="size-3" /> : <ChevronUp className="size-3" />} Level 2 · Time & Sales
          </button>
        </div>

        {/* AI side panel */}
        <aside className="w-80 shrink-0 border-l border-border bg-surface overflow-auto">
...
function L2Book({ price, side }: { price: number; side: "bid" }) {
  const rows = Array.from({ length: 10 }).map((_, i) => {
    const bidPrice = +(price - 0.01 - i * 0.02).toFixed(2);
    const askPrice = +(price + 0.01 + i * 0.02).toFixed(2);
    const bidSize = Math.floor(2400 - i * 180 + Math.random() * 400);
    const askSize = Math.floor(2200 - i * 160 + Math.random() * 400);
  ...
  return (
    <div className="overflow-auto">
      ...
      {rows.map((r, i) => (
        <div key={i} className="grid grid-cols-4 text-xs font-mono tabular-nums px-3 py-1 border-b border-border/30 relative">
          <div className="relative"><div className="absolute inset-y-0 left-0 bg-bull/10" style={{ width: `${(r.bidSize / maxSize) * 100}%` }} /><span className="relative">{r.bidSize}</span></div>
          <span className="text-bull">{r.bidPrice}</span>
          <span className="text-bear">{r.askPrice}</span>
          <div className="relative text-right"><div className="absolute inset-y-0 right-0 bg-bear/10" style={{ width: `${(r.askSize / maxSize) * 100}%` }} /><span className="relative">{r.askSize}</span></div>
        </div>
      ))}
    </div>
  );
...
function TimeSales({ ticker, basePrice }: { ticker: string; basePrice: number }) {
  ...
  const rowsRef = useRef<{ time: string; price: number; size: number; side: "B" | "S" }[]>([]);
  ...
  return (
    <div className="overflow-auto border-l border-border">
      ...
      {rowsRef.current.map((r, i) => (
        <div key={i} className="grid grid-cols-4 text-xs font-mono tabular-nums px-3 py-1 border-b border-border/30">
