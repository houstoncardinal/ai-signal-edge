...
import { Activity, Sparkles, TrendingUp, TrendingDown, Star, ArrowUpRight, Circle } from "lucide-react";
...
function Dashboard() {
  ...
  return (
    <div className="flex h-[calc(100vh-3rem)]">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {watchlist.map(t => {
                  ...
                  return (
                    <tr key={t} className="border-b border-border/60 hover:bg-accent/40">
          ...
          <Panel title="Recent AI Signals" right={<Link to="/app/signals" className="text-xs text-brand hover:underline">All signals →</Link>}>
            <div className="space-y-2.5">
              {SIGNALS.slice(0, 3).map(s => (
                <div key={s.id} className="rounded-md border border-border p-2.5 hover:border-brand/40 transition-colors">
                  <div className="flex items-center gap-2">
      ...
      {/* Right news rail */}
      <aside className="hidden xl:flex w-80 shrink-0 flex-col border-l border-border bg-surface">
        ...
        <div className="overflow-auto flex-1 p-3 space-y-2.5">
          {NEWS.map(n => (
            <article key={n.id} className="rounded-md border border-border p-2.5 hover:border-brand/40 cursor-pointer">
...
function KpiCard({ label, value, sub, icon: Icon, color }: { label: string; value: string; sub: string; icon: any; color: "brand" | "bull" | "warn" }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-3 flex items-center gap-3">
      <div className={`size-9 rounded-md flex items-center justify-center ${c}`}><Icon className="size-4" /></div>
      <div className="min-w-0">
        <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div>
        <div className="font-mono text-xl font-semibold tabular-nums leading-tight">{value}</div>
...
function MoverList({ rows, positive }: { rows: any[]; positive?: boolean }) {
  return (
    <ul className="divide-y divide-border/60">
      {rows.map(q => (
        <li key={q.ticker} className="flex items-center gap-3 py-2 first:pt-0 last:pb-0">
          ...
          {positive ? <TrendingUp className="size-3.5 text-bull" /> : <TrendingDown className="size-3.5 text-bear" />}
        </li>
      ))}
    </ul>
  );
